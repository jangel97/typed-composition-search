...
The \textbf{AAP MCP} domain is the official Ansible Automation Platform MCP Server\footnote{\url{https://github.com/ansible/aap-mcp-server}} (1,060 operations across four services), with its composition graph (50 entity types) derived automatically from OpenAPI specifications.
...
The benchmark contains 140 manually authored queries for the five benchmark domains (25--30 per domain) and 120 queries for the AAP domain.
...
\begin{itemize}
    ...
    \item \textbf{TCR + Trained Encoder}: a lightweight domain-specific classifier (frozen sentence encoder + dual classification heads, 475K parameters) replaces the LLM for entity type prediction. Evaluated on the AAP domain (Section~\ref{sec:production-eval}).
