...
Graph search then deterministically recovers the tool chain---compositional reasoning.
Where direct routing maps a query to a tool sequence ($q \rightarrow \{t_1, \ldots, t_k\}$), TCR factorizes this into type prediction ($q \rightarrow (e_{\mathrm{src}}, e_{\mathrm{tgt}})$) followed by graph path recovery ($(e_{\mathrm{src}}, e_{\mathrm{tgt}}) \rightarrow [t_1, \ldots, t_l]$).
...
\textbf{Right:} Typed Composition Routing decomposes the problem: the LLM performs semantic reasoning (entity type prediction), while graph search performs compositional reasoning (tool chain recovery), guaranteeing structural validity within the typed composition graph.
}
...
\item A \textbf{reformulation of tool routing} that decomposes it into semantic reasoning (entity type prediction) and compositional reasoning (deterministic graph search), replacing direct tool selection with a typed composition graph (Section~\ref{sec:method}).
