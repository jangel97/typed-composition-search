...
However, ControlLLM involves full task decomposition, parallel scheduling, and resource management---substantially more complex than our approach, which uses only type prediction and BFS.
...
Notably, GRAFT reports that constrained graph search achieves ``perfect dependency legality'' but dismisses it due to lower exact match.
...
Our typed graph provides deterministic composition guarantees that these approaches lack.

\paragraph{Positioning.}
Our work fills a gap between retrieval-based routing (no structural constraints, no training) and trained graph models (strong constraints, training required).
Typed Composition Search provides structural constraints---zero hallucinations, guaranteed compositional validity---without any training, making it the simplest structural approach in this space.
