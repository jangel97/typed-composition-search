...
We evaluate on five tool domains spanning infrastructure operations and e-commerce, chosen to cover diverse graph topologies (Table~\ref{tab:domain-stats}).
...
CI/CD is the critical test case: with 51 types for 54 tools (94\% ratio), the type space provides almost no label reduction, yet achieves the highest entity pruning (87\%).
This demonstrates that the graph constraint operates through topology, not label count.
...
Shopify tools are derived from the Shopify Admin REST API, providing a non-infrastructure domain for generalization evidence.
...
This is intentional: our method should work with any model that can classify entity types from natural language, including models not specifically trained for tool use.
...
We additionally report \textbf{hallucinated tool count}: tools returned by the model that do not exist in the registry.
