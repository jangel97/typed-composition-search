...
Graph-constrained entity prediction followed by graph search outperforms direct tool selection in end-to-end F1 and improves routing consistently across models and domains.

\paragraph{H2 (Graph Reduces Decision Space).}
Constraining tool routing to graph-reachable typed transformations reduces the effective decision space faced by the language model.
...
End-to-end recall decomposes into type prediction quality and graph reachability (Eq.~\ref{eq:recall-decomposition}), separating model quality from graph quality and enabling targeted failure analysis.
