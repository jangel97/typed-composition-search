...
contain hundreds of available tools \cite{qin2024toolllm}. Presenting the full
...
\cite{schick2023toolformer,patil2024gorilla}. While effective for small tool
...
User intent is typically expressed over domain objects and desired states;
executable tools are implementation details that users rarely reference directly.
...
This observation suggests that tool routing should operate over entity types
rather than tools directly.
A tool registry naturally induces a typed directed graph whose vertices
represent entity types and whose edges represent executable transformations.
Tool routing then becomes the problem of recovering a valid path between
...
(Section~\ref{sec:experimental-setup}). Rather, the graph itself constrains
the reasoning process: once a target entity type is predicted, reverse
reachability eliminates source types that cannot participate in any valid
composition (Section~\ref{sec:graph-constraint}).