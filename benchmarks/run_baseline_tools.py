...
Given a user query, call the tools needed to answer it."""
...
def tool_to_schema(tool) -> dict:
    return {
        ...
        "function": {
            "name": tool.name,
            "description": f"{tool.description} Input: {', '.join(tool.input_types)}. Output: {', '.join(tool.output_types)}.",
...
def select_tools(
    ...
    predicted = [tc.function.name for tc in tool_calls]
...
def run_baseline_tools(model_name: str, domain: str):
    ...
    tool_schemas = [tool_to_schema(t) for t in registry._tools]

    print(f"\n=== BASELINE-TOOLS: {model_name} ({model_id}) ===")
    print(f"Total tools as function schemas: {total_tools}\n")
    ...
    def process_query(q):
        valid_predicted, hallucinated, prompt_tok, completion_tok, latency_ms = select_tools(
            config, q["query"], tool_schemas, valid_names,
        )
        return {
    ...
    return {
        **report.base_result_dict("baseline-tools", hallucinated=all_hallucinated),
...
def main():
    ...
    for model_name in args.models:
        run_baseline_tools(model_name, args.domain)
