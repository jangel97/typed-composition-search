"""Generate training data for entity type prediction on AAP MCP.

Approach:
1. Load the frozen graph snapshot to get all tools with typed edges
2. Group tools by (source_type, target_type) pair
3. Generate canonical query templates from tool descriptions
4. Use an LLM to paraphrase templates across diverse styles
5. Generate hard negatives (same source, different target)
6. Output JSONL with (query, source_type, target_type, style)

The 47 benchmark queries in queries.py are the held-out test set
and must never appear in the training data.
"""
...
GRAPH_PATH = Path(__file__).parent / "graph_snapshot.json"

PARAPHRASE_PROMPT = """\
You are generating training data for an entity type classifier on Red Hat Ansible Automation Platform.
...
4. Question form
5. Vague/incomplete (avoid AAP-specific jargon, use everyday language)

Rules:
- Each query should be a realistic request from an AAP platform operator
- Vary the specific resource names (use names like "production", "staging", "web-server-01", "nightly-backup", "deploy-app", "engineering", etc.)
...
def load_test_queries() -> set[str]:
    from benchmarks.aap_mcp.queries import QUERIES
    return {q["query"].strip().lower() for q in QUERIES}


def group_tools_by_pair(
    graph: dict,
...
def run(model_name: str, dry_run: bool = False, n_per_pair: int = 15, n_neg: int = 5):
    graph = load_graph()
    ...
    # Find sources with multiple targets (for hard negatives)
    ...
    # Step 1: Generate paraphrases for each pair
    ...
    # Write output
    out_path = Path(__file__).parent / "training_data.jsonl"
...
def main():
    ...
    parser.add_argument("--n-per-pair", type=int, default=15, help="Paraphrases per type pair")
    parser.add_argument("--n-neg", type=int, default=5, help="Hard negatives per source-target")
    args = parser.parse_args()

    run(args.model, dry_run=args.dry_run, n_per_pair=args.n_per_pair, n_neg=args.n_neg)
